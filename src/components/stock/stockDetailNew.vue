    ...
    <section class="stock-detail">
        ...
        <div class="stock-detail__info">
            <div class="stock-detail__info-tag">
               ...
               <span v-if="type==5" style="color: red">{{handleInStockStatus(detailInfo.informStockStatus)}}</span>
            </div>
            <el-form style="font-size: 12px;">
                <div style="display: flex;flex-wrap: wrap">
                    ...
                    <el-form-item v-if="type==0||type==1" class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;white-space: nowrap"
                                  label-width="80px" label="库存变动标签:">
                        <span>{{detailInfo.labelName}}</span>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;white-space: nowrap"
                ...
                <div v-if="type!=5" style="display: flex;">
                    <el-form-item v-if="type==0||type==1" class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                        ...
                        <div v-if="detailInfo.stockInformId" style="word-break: break-all"><span @click="viewDetail(detailInfo.stockInformId,5)" style="cursor: pointer;text-decoration: underline">{{detailInfo.informOrderNo}}</span><span v-if="type==0">（{{detailInfo.informStorehouseName}}：{{handleInStockStatus(detailInfo.informStockStatus)}}）</span></div>
                        <div v-else>无</div>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                        ...
                        <span v-if="detailInfo.stockInformId&&type==1">{{detailInfo.stockOutNum}}/<span :style="{'color':detailInfo.num != detailInfo.stockOutNum ? 'red' :''}">{{detailInfo.num}}</span></span>
                        <span v-else>{{detailInfo.num}}</span>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;" label-width="80px"
                        ...
                        <span v-if="detailInfo.stockInformId&&type==1">¥{{detailInfo.stockOutAmount}}/<span :style="{'color':detailInfo.amount != detailInfo.stockOutAmount ? 'red' :''}">¥{{detailInfo.amount}}</span></span>
                ...
                <div v-if="type==5" style="display: flex;">
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                        ...
                        <div style="word-break: break-all" v-if="detailInfo.stockOutId"><span @click="viewDetail(detailInfo.stockOutId,0)" style="cursor: pointer;text-decoration: underline">{{detailInfo.stockOutNo}}</span>（{{detailInfo.stockOutStorehouseName}}）</div>
                        <div v-else>无</div>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                                  label-width="86px" label="入库通知数:">
                        <span>{{detailInfo.stockOutNum}}</span>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;" label-width="116px"
                                  label="入库通知金额:">
                        <span>¥{{detailInfo.stockOutAmount}}</span>
                    </el-form-item>
                </div>
                <div v-if="type==5" style="display: flex;">
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                                  label-width="86px" label="关联入库单:">
                        <div style="word-break: break-all" v-if="detailInfo.stockInId"><span @click="viewDetail(detailInfo.stockInId,1)" style="cursor: pointer;text-decoration: underline">{{detailInfo.stockInNo}}</span></div>
                        <div v-else>未入库</div>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;margin-right: 32px;"
                                  label-width="86px" label="实际入库数:">
                        <span>{{detailInfo.stockInNo?detailInfo.stockInNum:'无'}}</span>
                    </el-form-item>
                    <el-form-item class="form-item" style="width: 280px;margin-bottom: 0;" label-width="116px"
                                  label="实际入库金额:">
                        <span v-if="detailInfo.stockInNo">¥{{detailInfo.stockInAmount}}</span>
                        <span v-else>无</span>
                    </el-form-item>
                </div>
            </el-form>
            ...
            <el-popover
                    ...
                    trigger="click">
                <div>
                    <div @click="edit" v-if="type!=2&&(detailInfo.reviewStatus==0||detailInfo.reviewStatus==3)&&!detailInfo.facMissionId&&!detailInfo.missionNodeReport"
                         style="cursor: pointer;line-height: 32px;border-bottom: 1px solid #E4E8F1;">修改单据
                    </div>
                    <!--detailInfo.missionNodeReport true是上报产生的库存单据 不能撤销-->
                    <div v-if="detailInfo.reviewStatus==2&&!detailInfo.missionNodeReport" @click="$refs['popover2'].doClose();cancelDialog=true" style="cursor: pointer;line-height: 32px;">申请反审</div>
                </div>
            </el-popover>
            <div class="btn-group">
                <i class="icon-cancel" v-if="detailInfo.cancel"></i>
                <nf-button v-if="type==5&&detailInfo.informStockStatus==0" type="success" @click="confirmInStorage(false)">确认入库</nf-button>
                <nf-button v-if="type==5&&(detailInfo.informStockStatus==1||detailInfo.informStockStatus==2)" @click="confirmInStorage(true)">修改入库数</nf-button>
                ...
                <nf-button v-show="!detailInfo.missionNodeReport&&type!=5&&(((type!=2&&(detailInfo.reviewStatus==0||detailInfo.reviewStatus==3))&&!detailInfo.facMissionId) || detailInfo.reviewStatus==2)" v-popover:popover2 width='105' @click="isFold=!isFold" class="show-more-btn">
                    更多操作
        ...
        <div class="stock-detail__table">
            <div class="yellow-border-title" style="margin-bottom: 8px;">
                <div class="title">
                    <span>{{type==2?(isAdjustIn?'入库':'出库'):text}}{{type!=5?'合计':''}}: {{type==2?(isAdjustIn?'+':'-'):''}}{{detailInfo.styleDTOList.length}}款/
                        <!--{{(type==1||type==4||(type==2&&isAdjustIn))?detailInfo.addNum:detailInfo.subtractNum}}件 -->
                        <!--金额: {{type==2?(isAdjustIn?'+':'-'):''}}¥{{(type==1||type==4||(type==2&&isAdjustIn))?detailInfo.addAmount:detailInfo.subtractAmount}}-->
                        {{Math.abs(detailInfo.num)}}件 金额: ¥{{detailInfo.stockOutAmount}}
                    </span>
                </div>
                <div><span v-if="type==2">{{isAdjustIn?'入库':'出库'}}</span>仓库: {{type==5?detailInfo.informStorehouseName:detailInfo.storehouseName}}</div>
            </div>
            <storage-table
                    :isShowInformNum="(type==0&&detailInfo.stockInId)||(type==1&&detailInfo.stockInformId)||(type==5&&(detailInfo.informStockStatus==1||detailInfo.informStockStatus==2))"
            ...
            <nf-button :loading="btnLoading" v-if="type==5&&detailInfo.informStockStatus==0" @click="handleClose" style="margin-top: 8px;">关闭</nf-button>
...
<script>
    ...
    import {stockBillUndo,stockChangeDetail,stockUndo,reviewBillStock,stockBillSubmit,billReviewConfig,centralStockDetail,stockInformClose} from '@/api'
    ...
    export default {
        ...
        methods: {
            handleClose() {
                ...
                stockInformClose({id:this.id}).then(res => {
            ...
            viewDetail(id,type) {
              this.$router.replace(`/stockDetailNew?id=${id}&type=${type}&storehouseId=${this.$route.query.storehouseId}`)
              this.getDetail()
            },
            ...
            getLengths(skuList) {
                ...
                for (let skuItem of skuList) {
                    ...
                    !had && arr.push({value: skuItem.standardName || '无', id: skuItem.standardId});
                }
                return arr;
            },
            setTableData(data = {}) {
                ...
                for (let item of commonStyleSkuList) {
                    ...
                    skuValue[skuIdStr] = {
                        num: item.quantity,
                        informedQuantity: item.informedQuantity,
                        skuId: item.skuId,
                    ...
                    if (item.standardId && !json[item.standardId]) {
                        lengths.push({
                            standardValue: item.standardName || '无',
                    ...
                    // const idStr = item.sizeValue + (item.standardId ? '/' + item.standardName : '')
                    if (!json[item.sizeId]) {
                        sizes.push({
                            ...item,
                            sizeNum: '',
            ...
            getTableDataAndSkuValue(data, v, type, isIn) {
                ...
                let stockNum = v.skuDTOList.reduce((total,curV) => {
                    return total + (curV.informedQuantity || curV.quantity)
                },0)
                // if(type==2&&isIn) {
            ...
            confirmInStorage(type) {
                // 权限
                if(!this.powerJudgement.permissions("fac.stock.change.bill.update")&&type){
                    return
                }

                // 通知单 确认入库/修改入库
                this.editDialog = true;
                this.isEditNotice = type;
                ...
                this.$nextTick(() => {
                    ...
                    const bol = this.detailInfo.informStorehouseId != this.$route.query.storehouseId
                    if(type){
                        this.$refs['inStorage'].getStockList([this.id],bol?this.detailInfo.informStorehouseId:'');
                    }else {
                        if(bol){
                            this.$refs['inStorage'].getStockList([],this.detailInfo.informStorehouseId);
                        }
                    }
                })
            },
            edit() {
                ...
                console.log(this.tableData,'=3=3=3=3');
                // this.$set(this.editInfo,'inSkuValue',this.inSkuValue)
                // this.$set(this.editInfo,'outSkuValue',this.outSkuValue)
                this.$set(this.editInfo,'skuValue',this.skuValue)
                // this.$set(this.editInfo,'outTableData',this.outTableData)
                // this.$set(this.editInfo,'inTableData',this.inTableData)
                this.$set(this.editInfo,'tableData',this.tableData)
                this.$set(this.editInfo,'detail',this.detailInfo)
                this.$nextTick(() => {
                    this.$refs['inStorage'].getEditInfo();
                    if(this.detailInfo.stockInformId){
                        this.$refs['inStorage'].getStockList([this.detailInfo.stockInformId]);
                    }
                })
            },
            getDetail() {
               ...
               this.loading = true
                ...
                //     this.loading = false
                // })
                centralStockDetail({stockId:this.id,unitId:this.unitId}).then(res => {
