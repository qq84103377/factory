/*
 * @Author: kuhn 
 * @Description: 设计档案详情 - 计划成本核算
 * @Date: 2019-05-14 14:26:54 
 * @Last Modified by: kuhn
 * @Last Modified time: 2019-05-17 11:29:27
 */
  ...
  <div class="cost-accounting">
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-input
            :class="{'nameRepact':scope.row.nameRepact}"
            class="cost-accounting-table-name"
            v-else
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-select
            ...
            :placeholder="scope.row.edit?'请选择BOM模板':''">
            <el-option style="display: none" label="" value="">
            </el-option>
            <p
              @click="chachCanNewBom(scope.$index)"
              v-if="!designId"
              style="height: 30px;line-height: 30px;padding-left: 10px;color: #ffa914;cursor: pointer">新建BOM</p>
            <el-option
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-select
            ...
            :placeholder="scope.row.edit?'请选择工序模板':''">
            <el-option style="display: none" label="" value="">
            </el-option>
            <p
              @click="chachCanNewTemplate(scope.$index)"
              v-if="!designId"
              style="height: 30px;line-height: 30px;padding-left: 10px;color: #ffa914;cursor: pointer">新建工序模板</p>
            <el-option
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div class="table-operation">
            <div>
              <span  v-if="!scope.row.edit" @click="goDetail(scope.row)">查看详情</span>
    ...
    <transition name="opcity">
      <div
        class="cover1"
        v-if="createBomTableVisible"
        @click="createBomTableVisible = false">
      </div>
    </transition>
    <!--新建弹窗-->
    <transition name="right-move">
      <procedure-create
        :show="isShow"
        :editId="editId"
        @toggle="addSampleTemplate"
        v-if="isShow"></procedure-create>
    </transition>
    <transition name="opcity">
      <div
        class="cover"
...
export default {
  ...
  methods: {
    goDetail(row) {
      this.$router.push({
        path: '/designFile/costDetail',
        query: {
          id: row.id,
          bomId: row.bomId,
          type: 'designFile',
          designStyleId: this.$route.query.designStyleId || this.designId,
        }
      })
    },
    // 获取bom表格数据
    ...
    getList() {
      this.loading = true
      sampleCostList({
        splDesignStyleId: this.$route.query.designStyleId || this.designId,
        userId:this.userId
      }).then(res => {
        this.loading = false
        console.log(res.data.data, '成本核算列表');
    ...
    sampleSetDefault(id, name) {
      if (this.designId) {
        //工作台检查必填弹窗放那边
        this.$emit('rowSampleSetDefault', id, name)
      } else {
        this.isUpdateDefaultVisible = true;
    ...
    dialogSureGo() {
      if (this.dialogType === 'default') {
        return new Promise(resolve => {
          ...
          if (this.designId) {
            //工作台检查必填弹窗放那边
            this.$emit('dialogOperationUpdatePrice', this.sampleSetDefaultName)
          }
          this.dialogType = 'updatePrice';
          this.dialogTitle = '更新成本信息';
          this.dialogText = '是否将' + this.sampleSetDefaultName + '的总成本，更新到设计档案的计划成本价？';
        })
