  ...
  <div class="edit-artorder">
    <header class="header">
      ...
      <span class="header_right">
                <nf-button style="width: 98px;" @click="cancelAdd">取消</nf-button>
    ...
    <section>
        ...
        <div class="arr-artorder">
          <div class="header-artorder">
            <div class="header-artorder-right">
              <el-form :label-position="labelPosition" label-width="100px" :model="processForm"
                       :rules="rules" ref='ruleFrom'>
          ...
          <div class="ue-editor">
            <UE :defaultMsg='defaultMsg' :config=config id="uedd" ref="ue22"></UE>
          </div>
          <div>
            <attachEdit @attachD='attachList' :attachData='artData.attachments'></attachEdit>
          </div>
        </div>
        <!--<nf-button style="font-size:14px;border:1px solid #dce0e6;color:#fa4150;" @click="isdelete"-->
      ...
      <el-dialog :modal=false class="repertory_dialog repertory_dialog--dele" :visible.sync="deleteFac">
        ...
        <div class="footer">
          ...
          <nf-button type="warning" @click="deleteCommonProcessForm">确定</nf-button>
        </div>
      </el-dialog>
    </section>
  </div>
...
<script>
  ...
  export default {
    props: ["artData", "styleId",'sampleId','goodsStyleNo'],
    data() {
      return {
        labelPosition: "right",
        rules: {
    ...
    methods: {
      getGoodsStyleNo(){
          if(this.goodsStyleNo){
              this.processForm.name = this.goodsStyleNo;
          }else{
      ...
      isdelete() {
        var datalist = JSON.parse(sessionStorage.getItem("user_login"))
          .permissionList;
        if (datalist.indexOf("fac.goods.process.delete") == -1) {
      ...
      deleteCommonProcessForm() {
        ...
        deleteCommonProcessForm(params).then(res => {
          if (res.data.code == 0) {
      ...
      updateCommonProcessForm() {
        let content = this.$refs.ue22.getUEContent();
        this.processForm.content = content;
        let params = {
        ...
        addProcessForms(params).then(res => {
          if (res.data.code == 0) {
              this.processForm.content=''
              this.$refs.ue22.setUEContent('');
              this.processForm.name=''
              this.$emit("toggle");
          }
        });
