/*
 * @Author: kuhn
 * @Description: 全局设置 - 自动编码管理 - 款号编号规则 - 自定义规则弹窗
 ...
 * @Date: 2019-05-11 11:11:50
 * @Last Modified by: kuhn
 * @Last Modified time: 2019-05-11 14:21:18
 */

<template>

  <el-dialog class="custom-dialog auto-num-dialog"
             title="自定义规则"
             :visible.sync="centerDialogVisible"
             :modal-append-to-body="false"
             @close="emitEvent('closeDialog',changeType)"
             size="tiny">
              ...
              <transition-group>
                <div v-for="(item,index) of attrs"
                  ...
                  <div style="border-right:1px solid #e6eaf0;" class="color-title-name title-child-setting inputBox">
                    ...
                    <el-input v-else-if="item.type==1"
                              v-model="item.value"
                              placeholder="请输入位数"
                              style="border:0;"
                              :key="index+'1'"
                    ...
                    <span v-if="item.type==1 && item.value" class="child-setting-unit">位</span>
                  </div>
                  <div class="color-title-name title-operation color-opera handle">
                    ...
                    <div class="color-sort" @click="emitEvent('deleteRule',{changeType:changeType, index:index})">
  ...
  export default {
    ...
    methods: {
      ...
      checkCommitData() {
        ...
        this.$emit('sureCommit', this.changeType, this.beseemSelectArr)
      },
      addAttrToData() {
        for (let index in this.attrs) {
